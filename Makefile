...
# revise from xilinx helloworld-xrt example
...
COMMON_REPO ?= $(shell bash -c 'export MK_PATH=$(MK_PATH); echo $${MK_PATH%host_xrt/submission/*}')
...
HOST_SRCS += $(XF_PROJ_ROOT)/common/includes/cmdparser/cmdlineparser.cpp $(XF_PROJ_ROOT)/common/includes/logger/logger.cpp ./llama_xrt/src/llama2.cpp
...
$(TEMP_DIR)/forward.xo: llama_xrt_kernels/src/forward.cpp
	mkdir -p $(TEMP_DIR)
	$(VPP) $(VPP_FLAGS) -c --profile.data all -k forward --temp_dir $(TEMP_DIR)  -I'$(<D)' -o'$@' '$<'
